...
form.addEventListener('submit', (e) => {
  ...
  //2y7aga b7otha fl input btkon string 7ata lw kant rqm fana 3ayz 27wlha l number 3shan 23rf 2t3aml m3aha fl array w kda w h7wlha 3n tre2 parseInt

  //emptyValue solution
  //more than 9 solution(num of paragraphs i have in the array)
  //-ve number solution
  console.log(text.length);

  if (isNaN(amount.value) || amount.value < 0 || amount.value > text.length) {
    const random = Math.floor(Math.random() * text.length);
    loremText.innerHTML = `<p class='result'>${text[random]}</p>`;
  } else {
    //hena 3ayz 23ml ene 25le el array 2gb mnha bs 2wel kam element 3ala 7sb l amount ele el user d5lha f hst3ml slice 3shan 2qt3 goz2 mn el array mo3yn
    const finalText = text.slice(0, amount.value);
    console.log(finalText);
    loremText.innerHTML = finalText
      .map((ele) => {
        return `<p class='result'>${ele}</p>`;
      })
      .join('');
  }
});
